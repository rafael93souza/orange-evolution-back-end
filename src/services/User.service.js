...
const createTrailsUser = async (id, cursos) => {
    ...
    for (const trailID of cursos) {
        ...
        if (!trailExist.length) throw errors(400, 'Algum curso não foi encontrado no sistema!');
        const userChooseTrail = await knex("escolha").where({ usuario_id: id }).andWhere({ curso_id: trailID });
        if (!userChooseTrail.length) {
            const chooseTrail = await knex("escolha").insert({ usuario_id: id, curso_id: trailID }).returning("*")
